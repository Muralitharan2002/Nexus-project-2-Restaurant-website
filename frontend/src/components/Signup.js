...
function Signup() {
    ...
    const submitLogin = (e) => {
        ...
        if (Object.keys(error).length === 0) {
            setloader(true)
            axios.post("http://localhost:8000/router/login", { Email1, Password1 }, { withCredentials: true })
                .then((res) => {
    ...
    const submitSignup = (e) => {
        ...
        if (Object.keys(Error).length === 0) {
            setloader(true)
            axios.post("http://localhost:8000/router/signup", { Name, Email, Password }, { withCredentials: true })
                .then((res) => {
                    if (res.data.status === "success") {
